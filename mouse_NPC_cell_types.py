...
outdir = 'mouse_NPC_cell_types/results'
...
sample_clusters_ap = spectral_clustering(comp['PCmatrix'],12)
...
umap_comp = umap_emb(rpkm_df.T, sigVarGenes, n_comp=2, 
                annot=metadata.loc[rpkm_df.columns]['indv'], 
                annoGE=None, init='pca', init_n_comp=n_comp,
                n_neighbors=15, metric='euclidean', min_dist=0.5, 
                initial_embed='spectral',
                log=True, markerPlot=True, pcPlot=True, pcPlotType='normal', 
                ...
                with_std=False, figsize=(5,5), 
                outdir=outdir, legend_loc='right',
                filename1='umap_color_by_indv')

new_cell_type_annot = cell_cluster_annot.copy()

new_cell_type_annot['sampleCluster'] = new_cell_type_annot['sampleCluster'].replace('cluster12','NK cells',regex=True)
new_cell_type_annot['sampleCluster'] = new_cell_type_annot['sampleCluster'].replace('cluster11','CD8+ T cells',regex=True)
new_cell_type_annot['sampleCluster'] = new_cell_type_annot['sampleCluster'].replace('cluster10','Erythrocytes',regex=True)
new_cell_type_annot['sampleCluster'] = new_cell_type_annot['sampleCluster'].replace('cluster9','pDCs',regex=True)
new_cell_type_annot['sampleCluster'] = new_cell_type_annot['sampleCluster'].replace('cluster8','Hepatocytes',regex=True)
new_cell_type_annot['sampleCluster'] = new_cell_type_annot['sampleCluster'].replace('cluster7','Neutrophils',regex=True)
new_cell_type_annot['sampleCluster'] = new_cell_type_annot['sampleCluster'].replace('cluster6','MoMacs and cDCs',regex=True)
new_cell_type_annot['sampleCluster'] = new_cell_type_annot['sampleCluster'].replace('cluster5','Kupffer cells',regex=True)
new_cell_type_annot['sampleCluster'] = new_cell_type_annot['sampleCluster'].replace('cluster4','Endothelial cells 2',regex=True)
new_cell_type_annot['sampleCluster'] = new_cell_type_annot['sampleCluster'].replace('cluster3','B cells',regex=True)
new_cell_type_annot['sampleCluster'] = new_cell_type_annot['sampleCluster'].replace('cluster2','Endothelial cells 1',regex=True)
new_cell_type_annot['sampleCluster'] = new_cell_type_annot['sampleCluster'].replace('cluster1','CD4+ T cells',regex=True)
...
if not os.path.exists('%s/DEGs_mouse_cell_types' %(outdir)): os.makedirs('%s/DEGs_mouse_cell_types' %(outdir))
new_cell_type_annot.to_csv('%s/DEGs_mouse_cell_types/sample_cluster.csv' %(outdir), sep="\t", index=False)

grpList = list(set(new_cell_type_annot['sampleCluster'].values))
grpList = ','.join(grpList)
    
diff_expr('%s/rpkms.csv' %(indir), '%s/DEGs_mouse_cell_types' %(outdir), 
          '%s/DEGs_mouse_cell_types/sample_cluster.csv' %(outdir), grpList, 'kw', 0.01, 5000, True, 3, 
          'v2', 1, 'conover', False, 'mean', False)
